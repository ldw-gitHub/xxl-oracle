...
public class XxlJobServiceImpl implements XxlJobService {
	...
	public Map<String, Object> pageList(Paging page, Long jobGroup, String jobDesc, String executorHandler, String filterTime) {
        //执行器主键ID
		jobGroup = null == jobGroup ? -1:jobGroup; 
		// page list
		if (null != page) {
			PageHelper.startPage(page.getPageNum(), page.getPageSize(), page.getOrderBy());
		}
		List<XxlJobInfo> list = xxlJobInfoDao.pageList(jobGroup, jobDesc, executorHandler);
		PageInfo<XxlJobInfo> pageInfo =new PageInfo<>(list);
		//int list_count = xxlJobInfoDao.pageListCount(jobGroup, jobDesc, executorHandler);
		...
		if (StringUtils.isNotBlank(jobInfo.getChildJobId())) {
			...
			for (String childJobIdItem: childJobIds) {
				if (StringUtils.isNotBlank(childJobIdItem) && StringUtils.isNumeric(childJobIdItem)) {
					XxlJobInfo childJobInfo = xxlJobInfoDao.loadById(Long.valueOf(childJobIdItem));
	...
	public ReturnT<String> remove(Long id) {
		...
		try {
			...
			xxlJobInfoDao.delete(xxlJobInfo);
